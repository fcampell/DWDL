...
def facade_visibility_quad_polygon(
    ...
    Build a 4-vertex polygon:
      ...
      apex = two 45° rays of length distance_m from endpoints.
    """
...
def main():
    ...
    input_key = os.getenv("INPUT_KEY", "input/fassaden_zuerich.gpkg")
    output_key = os.getenv("OUTPUT_KEY", "silver/facade_visibility.gpkg")
    ...
    gdf = gdf.head(10)  # ⚠ still limiting to 10 rows – remove or parametrize for full run
    logger.info("Loaded %d facades", len(gdf))
    logger.info("Input CRS: %s", gdf.crs)

    original_crs = gdf.crs

    # Optional: reproject for consistent metric geometry
    ...
    # ~20 log messages max → every ~5% of rows, at least every 1 row
    log_every = max(1, total // 20)

    logger.info("Computing visibility polygons for %d facades (log every %d rows)...",
                total, log_every)
    ...
    # --------------------------------------------------------
    # Write GPKG with single geometry column
    # --------------------------------------------------------
